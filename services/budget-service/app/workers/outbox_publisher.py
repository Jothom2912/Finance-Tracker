"""Outbox publisher for budget-service.

Polls outbox_events for pending rows, publishes them to RabbitMQ,
and marks them as published.  At-least-once delivery by design:
if the process crashes between publish and mark_published, the row
stays pending and is re-published on next poll — the consumer-side
source_key dedup handles the duplicate.
...
from app.adapters.outbound.postgres_outbox_repository import OutboxEntry, PostgresOutboxRepository
...
class OutboxPublisherWorker:
    ...
    async def run_forever(self) -> NoReturn:
        logger.info(
            "Budget outbox publisher started (poll=%.1fs, batch=%d)",
            self._poll_interval,
            self._batch_size,
        )
        while True:
    ...
    async def _try_publish(self, repo: PostgresOutboxRepository, entry: OutboxEntry) -> None:
        try:
            ...
            logger.warning(
                "Failed to publish %s (id=%s) — retry in %ds",
                entry.event_type,
                entry.id,
                backoff,
                exc_info=True,
