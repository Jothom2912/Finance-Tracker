"""Outbox publisher for budget-service — thin shim over finans-tracker-messaging.

At-least-once delivery by design: if the process crashes between publish and
mark_published, the row stays pending and is re-published on next poll — the
consumer-side source_key dedup handles the duplicate.
