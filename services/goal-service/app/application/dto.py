...
class GoalBase(BaseModel):
    name: Optional[str] = Field(default=None, max_length=45, description="Goal name")
    ...
    target_date: Optional[date] = Field(default=None, description="Deadline for goal (must be future date)")
    status: Optional[str] = Field(default=None, description="Goal status (e.g., 'active', 'completed')")
    ...
    def validate_current_vs_target(cls, v: float, info) -> float:
        ...
        if "target_amount" in info.data:
            ...
            if v > target:
                raise ValueError(f"Current amount ({v}) kan ikke være større end target amount ({target})")
        return v
    ...
    def validate_target_date(cls, v: Optional[date]) -> Optional[date]:
        ...
        if v is not None:
            today = date.today()
            if v <= today:
                raise ValueError(f"Deadline skal være i fremtiden. Got: {v}, Today: {today}")
        return v


# --- Create Schema ---
class GoalCreate(GoalBase):
    Account_idAccount: int = Field(..., description="Account ID")
