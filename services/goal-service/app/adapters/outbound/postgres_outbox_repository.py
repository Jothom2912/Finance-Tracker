...
class PostgresOutboxRepository(IOutboxRepository):
    ...
    async def fetch_pending(self, batch_size: int = 10) -> list[OutboxEntry]:
        ...
        stmt = (
            ...
            .where(OutboxEventModel.status.in_(["pending", "failed"]), OutboxEventModel.next_attempt_at <= now)
            .order_by(OutboxEventModel.created_at)
            .limit(batch_size)
            .with_for_update(skip_locked=True)
        )
        result = await self._session.execute(stmt)
        return [self._to_entry(model) for model in result.scalars().all()]
    ...
    async def mark_failed(self, event_id: str, next_attempt_at: datetime) -> None:
        await self._session.execute(
            update(OutboxEventModel)
            .where(OutboxEventModel.id == event_id)
            .values(status="failed", attempts=OutboxEventModel.attempts + 1, next_attempt_at=next_attempt_at)
        )
