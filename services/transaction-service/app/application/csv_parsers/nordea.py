...
_REQUIRED_COLUMNS = {"Bogføringsdato", "Beløb", "Beskrivelse"}


class NordeaCSVParser:
    """Parser for Nordea netbank CSV exports.

    Expected format:
    - Encoding: UTF-8 (possibly with BOM) or Windows-1252
    - Separator: semicolon
    - Columns: Bogføringsdato;Beløb;Afsender;Modtager;Navn;Beskrivelse;Saldo;Valuta;Afstemt
    ...
    def parse(
        ...
        for row_num, row in enumerate(reader, start=2):
            ...
            try:
                tx_date = self._parse_date(row["Bogføringsdato"])
    ...
    def _extract_description(row: dict) -> str:
        beskrivelse = (row.get("Beskrivelse") or "").strip()
        if beskrivelse:
            return beskrivelse
