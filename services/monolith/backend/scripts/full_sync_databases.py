...
from backend.config import ELASTICSEARCH_HOST, NEO4J_PASSWORD, NEO4J_URI, NEO4J_USER
...
def sync_elasticsearch():
    ...
    try:
        ...
        es.indices.create(
            index="accountgroups",
            body={"mappings": {"properties": {"idAccountGroups": {"type": "integer"}, "name": {"type": "text"}}}},
        )
        ...
        for category in categories:
            es.index(
                ...
                body={"idCategory": category.idCategory, "name": category.name, "type": category.type},
            )
        ...
        for ag in account_groups:
            es.index(
                ...
                body={"idAccountGroups": ag.idAccountGroups, "name": ag.name},
            )
...
def sync_neo4j():
    ...
    try:
        ...
        with neo4j_driver.session() as session:
            ...
            for ag in account_groups:
                session.run(
                    "CREATE (ag:AccountGroup {idAccountGroups: $id, name: $name})", id=ag.idAccountGroups, name=ag.name
                )
            print(f"   ✓ {len(account_groups)} kontogrupper")
