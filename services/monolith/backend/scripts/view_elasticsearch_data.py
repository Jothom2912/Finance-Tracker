...
def view_transactions(limit=10):
    ...
    result = es.search(
        index="transactions", body={"query": {"match_all": {}}, "size": limit, "sort": [{"date": "desc"}]}
    )
...
def view_categories():
    ...
    result = es.search(index="categories", body={"query": {"match_all": {}}, "size": 100})
    ...
    for hit in result["hits"]["hits"]:
        doc = hit["_source"]
        rows.append([doc.get("idCategory", "N/A"), doc.get("name", "N/A"), doc.get("type", "N/A")])
...
def view_accounts():
    ...
    result = es.search(index="accounts", body={"query": {"match_all": {}}, "size": 100})
    ...
    if indices_info.get("categories", 0) > 0:
        ...
        try:
            cat_type_agg = es.search(
                index="categories",
                body={"size": 0, "aggs": {"by_type": {"terms": {"field": "type.keyword", "size": 10}}}},
            )
    ...
    if indices_info.get("budgets", 0) > 0:
        ...
        try:
            budget_agg = es.search(
                ...
                body={
                    "size": 0,
                    "aggs": {"total_budget": {"sum": {"field": "amount"}}, "avg_budget": {"avg": {"field": "amount"}}},
                },
            )
