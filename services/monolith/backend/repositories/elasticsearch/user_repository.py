...
class ElasticsearchUserRepository(IUserRepository):
    ...
    def get_all(self) -> List[Dict]:
        ...
        try:
            response = self.es.search(index=self.index, query={"match_all": {}}, sort=[{"idUser": "asc"}])
            users = []
    ...
    def get_by_username(self, username: str) -> Optional[Dict]:
        ...
        try:
            response = self.es.search(index=self.index, query={"term": {"username": username}})
    ...
    def create(self, user_data: Dict) -> Dict:
        ...
        if "idUser" not in user_data or user_data.get("idUser") is None:
            try:
                ...
                response = self.es.search(index=self.index, size=0, aggs={"max_id": {"max": {"field": "idUser"}}})
        ...
        try:
            self.es.index(index=self.index, id=doc["idUser"], document=doc)
    ...
    def get_by_email_for_auth(self, email: str) -> Optional[Dict]:
        ...
        try:
            response = self.es.search(index=self.index, query={"term": {"email": email}})
            if response["hits"]["total"]["value"] > 0:
                source = response["hits"]["hits"][0]["_source"]
                return {
                    "idUser": source.get("idUser"),
