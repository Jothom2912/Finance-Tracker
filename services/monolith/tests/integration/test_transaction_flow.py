...
class Factory:
    ...
    def budget(db: Session, account_id: int, category_id: int, amount: Decimal) -> Budget:
        budget = Budget(amount=amount, budget_date=date.today().replace(day=1), Account_idAccount=account_id)
        db.add(budget)
        db.flush()
        db.execute(
...
class TestTransactionCreation:
    ...
    def test_create_transaction_updates_budget(self, test_client, test_db, mock_repositories):
        ...
        assert found_category is not None, (
            f"Category {category.idCategory} should exist but was not found by repository"
        )
