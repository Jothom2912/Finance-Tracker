...
def should_notify_bank_sync(*, trigger: SyncTrigger, new_imported: int, errors: int, parse_skipped: int = 0) -> bool:
    ...
    "Nothing to report" means all three failure counters are zero.
    ``parse_skipped`` matters as much as ``errors``: a sync that fetched 40
    transactions and could not read one of them has ``new_imported == 0`` and
    ``errors == 0``, but it is broken, not quiet. Suppressing it would make a
    dead bank connection indistinguishable from a healthy night.
    """
