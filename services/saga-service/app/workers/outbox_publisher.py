...
class OutboxPublisherWorker:
    ...
    async def run_forever(self) -> None:
        logger.info("Saga outbox publisher started (poll=%.1fs, batch=%d)", self._poll_interval, self._batch_size)
        while True:
    ...
    async def _try_publish(self, repo: PostgresOutboxRepository, entry: object) -> None:
        try:
            message = Message(
