...
- [findings/2026-07-25-transaction-hard-delete-categorized-dlq.md](findings/2026-07-25-transaction-hard-delete-categorized-dlq.md) — transactions are hard-deleted, so the categorization write-back cannot tell "not yet" from "gone" and retries a deleted row to the DLQ (MEDIUM, **resolved 2026-07-28** af P2-25 + P3-37 — tre af finding'ens egne påstande blev korrigeret af målingen)
...
- [findings/2026-07-27-goal-entity-two-runtime-types.md](findings/2026-07-27-goal-entity-two-runtime-types.md) — `Goal` konstrueres som `float` af det ene repository og `Decimal` af det andet; forskellen når ud i event-payloads som `"100.0"` mod `"100.00"` (MEDIUM, → P2-34).
- [findings/2026-07-27-retry-header-read-five-ways.md](findings/2026-07-27-retry-header-read-five-ways.md) — `x-retry-count` læses fem steder på fire måder; de to saga-consumere kaster `TypeError` på en `str`-header, og fordi læsningen står inde i retry-handleren bliver det en uendelig redelivery (MEDIUM, transaction rettet, resten → P2-36).
- [findings/2026-07-27-none-annotation-204-fastapi-split.md](findings/2026-07-27-none-annotation-204-fastapi-split.md) — et `-> None` tilføjet for typecheck-gaten dræbte budget-services container ved import, fordi `from __future__ import annotations` gør annotationen til `NoneType` og FastAPI 0.115.0 læser det som en response model på en 204; testene importerede det samme modul, bare under lockens nyere FastAPI (HIGH, rettet, rod → P2-37).
- [findings/2026-07-27-serializable-event-demands-mutable-attrs.md](findings/2026-07-27-serializable-event-demands-mutable-attrs.md) — `SerializableEvent` krævede settable attributter, så frozen `BaseEvent` opfyldte ikke sin egen Protocol; kun saga så det, fordi P2-32-ignoren skjulte det i de andre services (LOW, rettet i messaging 0.1.2).
- [findings/2026-07-27-optional-id-hides-unpersisted-entity.md](findings/2026-07-27-optional-id-hides-unpersisted-entity.md) — `id: Optional[int]` giver persisteret og upersisteret entitet samme type i 4 services; pydantic vagter de fleste kaldsteder, men `WHERE id IS NULL` gør ét af dem til en vildledende 409 (LOW, → P2-35).
...
- [plans/2026-07-28-p323-banking-uv-pyproject.md](plans/2026-07-28-p323-banking-uv-pyproject.md) — P3-23: banking-service på uv + pyproject + typecheck-gaten, så P2-31 dækker den service dens egen bug motiverede; `python-jose` viser sig kun at bruges i en test og forlader runtime som biprodukt (P3-26's banking-halvdel).
- [plans/2026-07-28-p225-transaction-soft-delete.md](plans/2026-07-28-p225-transaction-soft-delete.md) — P2-25 + P3-37: `deleted_at` på `transactions`, så `categorized_consumer` kan skelne "ikke committet endnu" fra "slettet for altid"; blast radius målt til transaction-service alene, fordi ES-projektionen allerede tombstoner og analytics allerede filtrerer.
- [plans/2026-07-28-p229-csv-upload-guards.md](plans/2026-07-28-p229-csv-upload-guards.md) — P2-29: byte-, række- og transportgrænse på `/import-csv`; målt at RAM-eksplosionen er `.read()` med tre samtidige kopier mod podens 512Mi, og at rækkegrænsen er den bindende i praksis, ikke byte-grænsen.
- [plans/2026-07-28-p324-datastore-loopback-bind.md](plans/2026-07-28-p324-datastore-loopback-bind.md) — P3-24's datastore-halvdel: de 14 datastore-mappings binder `127.0.0.1`; eksponeringen var målbar (642 transaktions-docs læsbare fra LAN uden auth), og backloggens "no downside" viste sig kun sandt for loopback-bind, ikke for at slette `ports:` — syv host-forbrugere bruger dem, heraf én i CI.
- [plans/2026-07-28-p343-nginx-perimeter.md](plans/2026-07-28-p343-nginx-perimeter.md) — P3-43 **done 2026-07-28**: ADR-0005 i drift (16 proxy-locations + deny-backstop, relative URLs, 11 × `CORSMiddleware` ud, rule 5 verificeret rød på 11 mutationer). Perimeteren kunne ikke bevises før planen tilføjede `frontend` til compose — den service var verifikationens forudsætning, ikke en bekvemmelighed. Åben ende: chat-SSE'ens pipeline (→ P3-46).
...
- [decisions/2026-07-28-transaction-soft-delete.md](decisions/2026-07-28-transaction-soft-delete.md) — P2-25 + P3-37: `deleted_at` på `transactions`; dedup og det partielle external_id-index ekskluderer tombstones (ellers blokerer en sletning re-import for altid), saga-rollback bliver soft, og consumeren **acker** en kategorisering for en slettet række frem for at DLQ'e den. Genoplivning afvist fordi ES-guarden er terminal.
- [decisions/2026-07-28-nginx-as-perimeter.md](decisions/2026-07-28-nginx-as-perimeter.md) — P3-24, promoted til [ADR-0005](../docs/adr/0005-nginx-as-security-perimeter.md): frontendens nginx bliver perimeteren, ikke gateway-service, som forbliver CQRS-læsesiden; beslutningen blev omformet af at k8s slet ikke har en indgang (30 ClusterIP, ingen Ingress) og `VITE_*` ikke er sat nogen steder — der er ingen deployment hvor en perimeter ville være nåelig.
