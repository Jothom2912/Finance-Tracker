...
- [findings/2026-07-25-per-worker-image-staleness.md](findings/2026-07-25-per-worker-image-staleness.md) — every worker carries its own `build:` block, so `compose build <svc>` leaves its workers on a stale image (MEDIUM, open).
