...
connect → callback (session created, connections stored) → sync request (outbox `saga.bank_sync.start`, correlation_id = fresh saga_id per request — double-click ⇒ two concurrent sagas) → start consumer creates instance → `saga.cmd.bank_fetch_transactions` → banking fetches from EB (sync client in async consumer!) and replies with **all transaction items inline** → items stored into saga context (re-serialized on every update; multi-MB rows/messages) → `saga.cmd.bulk_import_transactions` → transaction-service bulk import → `saga.cmd.mark_sync_complete` → banking sets `last_synced_at` → completed. Frontend polls saga status.
