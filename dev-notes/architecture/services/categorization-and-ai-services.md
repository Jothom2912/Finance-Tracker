...
updated: 2026-08-01
source: architecture audit 2026-07-07; F1-02/03 update 2026-07-17; TAX-06
---
...
- **Rule engine** *(TAX-06)*: 82 active global rules persist explicit evidence field/operator, direction, confidence, provenance and optional provider/country/amount constraints; merchant aliases are category-neutral. `ConstrainedRuleEngine` applies them without hidden sign overrides. Priority ladder **10 = learned / 50 = user / 100 = seed** remains, with cached per-user overlays and a 60s global TTL.
- **Taxonomy storage** *(TAX-06)*: migration 008 additively activates 13/67 target nodes with pinned UUIDv7 `public_id`, immutable semantic keys, version/lifecycle metadata and explicit fallbacks. The legacy 10/41 nodes remain soft-deprecated and integer-addressable for TAX-07. Migration and operator repair publish 80 deterministic v3 full-state snapshots through the outbox.
