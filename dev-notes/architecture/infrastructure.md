...
- **Compose** (primary dev runtime): redis, 9× postgres (one per service — incl. `postgres-notifications`, **host port 5441** since 5439 was taken by banking), rabbitmq, ollama (+ one-shot `ollama-pull`), 11 API services, 18 worker/consumer containers (incl. the two worker-loop schedulers `budget-month-close-scheduler` (F1-07) + `banking-sync-scheduler` (F1-05) — [pattern decision](../decisions/2026-07-17-scheduler-pattern-worker-loop.md) — and F1-01's `notification-service` + `notification-consumer`). Frontend NOT in compose (`make dev-frontend`).
