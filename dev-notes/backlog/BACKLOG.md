...
| P3-60 | **goal + account rapporterer upstream-nedetid som en brugerfejl.** `goal/account_adapter.py:24` og `account/user_adapter.py:26` kollapser en død eller afvisende upstream til "findes ikke" og svarer 400 — en besked klienten med rette aldrig genforsøger. P3-59 gjorde det *synligt* (begge steder logger nu hvorfor), men rettede ikke statuskoden: 503 er det ærlige svar, og det er en kontraktændring der rører frontendens fejlhåndtering. Bemærk at `goal`s `exists`-gren er uopnåelig i dag (se P3-62), så fixet dér er reelt i `get_owner_user_id` | goal, account | S | open | [P3-59s Non-goals](../plans/2026-07-31-p359-request-path-logging.md#items-der-spawnes-ikke-løses-her) |
...
| P3-62 | **`create_goal` gør to round-trips til account-service hvor den anden er død.** `service.py:92` kalder `_verify_ownership` → `get_owner_user_id`, som 404'er hvis kontoen ikke findes; `:94` kalder derefter `exists()` mod samme service. `exists()` kan kun returnere `False` hvis kontoen forsvandt mellem de to kald, og enhver upstream-fejl er allerede blevet en 503 på det første — målt live under P3-59 fase 5 i to fejlmoder. Altså et ekstra HTTP-kald pr. målsoprettelse hvis eneste opnåelige resultat er `True`. Fjernelsen flytter kilden til `AccountNotFoundForGoal` og er derfor en adfærdsændring | goal | S | open | [P3-59 fase 5](../plans/2026-07-31-p359-request-path-logging.md#fase-5s-resultat-2026-07-31) |
