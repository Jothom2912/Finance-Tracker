...
| P2-21 | k8s manifest drift: 6 workloads + 1 DB live in compose but have no manifest or `kustomization.yaml` entry. [→ detail](#p2-21) | infra | M | **done 2026-08-01** | [finding](../findings/2026-07-25-k8s-manifest-drift.md), [plan](../plans/2026-08-01-p221-p317-k8s-parity-migration-ordering.md) |
...
| P2-28 | Any authenticated user can mutate or delete the global taxonomy. [→ detail](#p2-28) | categorization | S | **done 2026-07-29** | [plan + Outcome](../plans/2026-07-29-p228-taxonomy-internal-only.md#outcome), [decision](../decisions/2026-07-29-taxonomy-authorization.md), [sweep SEC-5](../findings/2026-07-26-product-surface-sweep.md) |
...
| P2-42 | **Done 2026-07-29 (a+b).** a: `BankConfigError` → 503 + WARNING (app-level handler, fordi fejlen kastes under dependency-resolution og ingen rute-`try/except` kan nå den). b: `GET /ready` på banking rører DB og Enable Banking-konfigurationen, og en CI-gate læser den. **To niveauer, fordi afhængighederne ikke er samme slags:** DB påkrævet → 503, Enable Banking valgfri → **200 + `degraded`** (et 503 dér ville tage poden ud af service over en integration deployet måske ikke bruger). Gaten er med vilje strengere end proben — CI *bruger* bank-sync. Verificeret rød ved mutation mens `/health` og `compose_state_check.py` blev grønne. **Målt undervejs:** DB-niveauets 503 er ikke nåelig ved opstart (CMD er `alembic upgrade head && …`, så containeren restarter i stedet) — det er redundant ved *deploy*, unikt ved *drift* (DB-tab midt i kørsel, hvor de to andre gates er blinde) | banking, CI | S | done | [plan + Outcome](../plans/2026-07-29-p242b-dependency-readiness.md#outcome) · [finding](../findings/2026-07-28-banking-service-dead-in-ci.md) |
...
| P2-43 | **En klient kan gøre en goal-række permanent 500 med ét PUT.** `goal-service/app/application/dto.py:15` havde `status: Optional[str]` uvalideret, mens responsemodellen krævede enum; ugyldig input blev derfor committet før 500. Requesten accepterer nu kun lagret `active|paused`, og migration 006 reparerer gamle værdier og håndhæver invariant/default i DB'en | goal | S | **done 2026-08-01** | [plan + outcome](../plans/2026-08-01-p243-goal-status-integrity.md#outcome-fill-in-when-done) |
| P2-44 | TAX-06 migration 008 bruger faste surrogate-ID'er fra 11 og fejler på den faktiske snapshotkopi, hvor ID 11 allerede er allokeret; lav en fremadrettet, referencebevarende migrationsreparation og verificér den mod denne snapshotform | categorization, taxonomy, migrations | M | **done 2026-08-01** | [plan + outcome](../plans/2026-08-01-p244-tax06-surrogate-id-repair.md#outcome-fill-in-when-done) · [decision](../decisions/2026-08-01-p244-pre008-bootstrap.md) · [finding](../findings/2026-08-01-tax06-migration-collides-with-existing-category-ids.md) · [TAX-07 run](../plans/2026-08-01-tax07-existing-data-reclassification-dry-run.md#outcome-fill-in-when-done) |
...
| P3-21 | ai-service's eval seed writes 66 fixture documents into the production ES index. [→ detail](#p3-21) | ai, analytics, tests | S | **done 2026-08-01** | [plan + outcome](../plans/2026-08-01-p321-elasticsearch-eval-isolation-and-reconciliation.md#outcome-fill-in-when-done) · [finding](../findings/2026-07-26-eval-seed-writes-to-prod-index.md) |
...
| P3-28 | Root `.dockerignore` + rule 8, cachefrie uv-images og gzip/immutable frontend-assets; repræsentativt goal-image halveret fra 198,0 til 98,9 MB og live headers verificeret. [→ detail](#p3-28) | infra, frontend | S | **done 2026-08-01** | [plan + Outcome](../plans/2026-08-01-p328-build-image-hygiene.md#outcome-fill-in-when-done), [sweep OPS-1](../findings/2026-07-26-product-surface-sweep.md) |
...
| P3-46 | Compose henter nu alle tre AI-modeller deterministisk; med 16 GB Docker-memory gennemfører qwen3:8b-pipelinen både rå SSE og Chromium-E2E uden OOM. [→ detail](#p3-46) | ai, infrastructure | S | **done 2026-08-01** | [plan + Outcome](../plans/2026-08-01-p346-qwen8b-chat-e2e.md#outcome-fill-in-when-done) |
| P3-47 | `compose-check` afviser nu enhver nginx-`location` med egen `add_header`, medmindre den gentager alle fire security headers; negative og positive parserkontroller fastholder arvereglen. [→ detail](#p3-47) | infra, frontend | S | **done 2026-08-01** | [plan + Outcome](../plans/2026-08-01-p347-nginx-header-inheritance-gate.md#outcome-fill-in-when-done), [origin](../plans/2026-07-28-p325-p227-perimeter-headers-ratelimit.md#åbne-valg) |
...
| P3-49 | **Done 2026-07-29.** `-ll -ii` ind i `security`-targettet i gateway, banking og account, så lokal bandit og CI er samme kommando; `make -C services/gateway-service check` er grøn lokalt for første gang. **`# nosec B105` fravalgt, og begrundelsen i planen var forkert:** med `-ll` filtreres B105 (Low) alligevel, så nosec'en havde ikke skjult noget — den ægte grund er at nosec fixer *instansen* og ikke *klassen*, så næste Low-fund ville divergere igen. Reglen der kom ud af det, målt ved at fjerne den eksisterende nosec: **nosec er forbeholdt fund der overlever CI-tærsklen** (`B104` Medium rapporteres også med `-ll -ii` → load-bearing; `B105` Low filtreres → nosec ville være død annotation) | CI, cross, deps | S | done | [plan + Outcome](../plans/2026-07-29-p242b-dependency-readiness.md#outcome) |
| P3-50 | **Der findes ingen `.dockerignore`**, så hele arbejdstræet sendes til Docker-dæmonen som build-kontekst ved hvert `up --build` — inkl. `.git/`, `node_modules/`, alle 15 `.venv/` og den gitignorerede `enablebanking-sandbox.pem`. **Ikke en læk**, og det er verificeret frem for antaget: ingen Dockerfile bruger bred `COPY .`, og `find / -name "*.pem"` i banking-imaget giver kun CA-bundles. Men det er et manglende værn mod at en fremtidig bred `COPY` bager en credential ind, og det gør hver build langsommere end nødvendigt | infra, deps | S | open | målt under [P2-38](../plans/2026-07-29-p238-p242-ci-missing-signal.md#outcome) |
| P3-51 | Den lokale `enablebanking-sandbox.pem` står **`-rw-r--r--` (644)** — læsbar for enhver bruger på maskinen, hvor en privat nøgle bør være 600. **Men 644 er bærende, ikke sjusk, og det er verificeret:** containeren kører som `uid=10001 (appuser)` og læser nøglen via read-only bind-mount, så P2-39 måtte `chmod 644` i CI netop fordi `openssl genrsa`s default på 600 ejet af runneren gav `PermissionError`. Et blindt `chmod 600` brækker altså bank-sync. Det rigtige fix er **ejerskab frem for mode** (`chown 10001`, en matchende host-bruger, eller nøglen som secret frem for bind-mount) — og spørgsmålet er reelt "hvordan leverer vi en privat nøgle til en non-root container", ikke en permissions-oneliner | banking, infra | S | open | målt under [P2-38](../plans/2026-07-29-p238-p242-ci-missing-signal.md#outcome), mekanisme fra [P2-39s finding](../findings/2026-07-28-banking-service-dead-in-ci.md) |
| P3-52 | `analytics-service`s lokale venv kører **Python 3.14** mens CI kører **3.11** (`requires-python = ">=3.11"`, som tillader begge). Det er præcis formen på "virker lokalt, ikke i deploy": en 3.12+-only syntaks eller en stdlib-adfærdsændring ville være grøn lokalt og rød i CI, eller omvendt. Ikke undersøgt hvor mange af de 15 services der divergerer — start med at måle det, ikke med at pinne | cross, deps, CI | S | open | faldt over under [P2-38](../plans/2026-07-29-p238-p242-ci-missing-signal.md#outcome) trin 3 |
...
| P3-57 | **11 af 12 API-processer har ingen logging-konfiguration: `logger.info` når aldrig loggen, og `logger.warning` når den *uden* niveau, tidsstempel og logger-navn.** [→ detail](#p3-57) | cross, infra, observability | M | **done 2026-07-31** | målt under [F2-08](../plans/2026-07-29-f208-user-profile-write-path.md#outcome), [plan + Outcome](../plans/2026-07-31-p357-api-logging-config.md#outcome) |
| P3-58 | **`account-service`s API-proces er helt tavs — 4 logliner på 35 timers uptime, ingen access-log, mens containeren er `healthy` med `restarts=0`.** `alembic/env.py:20`s `fileConfig()` defaulter `disable_existing_loggers=True` og slukker uvicorns tre loggere, og `account` er den eneste service der kører migrations *i API-processen* (`main.py:33`, fra `lifespan`) frem for i CMD. Målt: `uvicorn.access disabled=False → True` hen over kaldet. Grep-verificeret: 9 af 9 `env.py` kalder `fileConfig`, kun `transaction`s sender `disable_existing_loggers=False` — fælden er ramt og lukket ét sted før uden at blive konvention. Fixes som step 2 i P3-57, fordi P3-57s eget `setup_logging()` ellers ville blive slukket igen i netop denne service. Latent landmine for P3-17, som flytter procesopdelingen | account, observability, cross | S | **done 2026-07-31** | [finding](../findings/2026-07-31-account-service-log-silenced-by-alembic.md), [plan + Outcome](../plans/2026-07-31-p357-api-logging-config.md#outcome) |
| P3-59 | **Fem af de 12 API-processer har ingen `logger.warning`/`error` overhovedet i request-stien, og `goal-service` har nul logging-statements i hele API-processen.** Målt under P3-57s før-måling: `account`, `user`, `goal`, `notification` og `saga` kan ikke sige noget om sig selv når en request går skævt — `notification` og `saga` har deres warnings i consumer-processen, `user` kun `logger.info`. Det er derfor P3-57 ikke kan verificeres adfærdsmæssigt i de fem: en logging-konfiguration hjælper ikke en proces der intet logger. Ikke et fix men et review: hvilke afvisninger og domænefejl i de fem services burde efterlade et spor, og på hvilket niveau | observability, cross | M | **done 2026-07-31** | [plan + Outcome](../plans/2026-07-31-p359-request-path-logging.md#outcome-2026-07-31) |
...
| P3-60 | **Account rapporterede upstream-nedetid som en brugerfejl; goals tilsvarende gren viste sig uopnåelig.** Account skelner nu user-services 404 → 400 fra request/non-404 → 503, og AccountSelector viser fejlen som midlertidig uden mutations-retry. Goal-halvdelen blev lukket sammen med P3-62 ved at fjerne den døde `exists()`-sti | goal, account, frontend | S | **done 2026-08-01** | [plan + Outcome](../plans/2026-08-01-p360-p362-upstream-error-honesty.md#outcome-fill-in-when-done) |
| P3-61 | **`clear_default_savings_goal` mangler `deleted_at`-filteret** (`goal/postgres_goal_repository.py:90-93`), hvor hver søsterstatement har det. Benignt i dag, men det er den inkonsistens der gør P3-16s invariant svær at stole på. Dertil: det partielle unique-index har ingen `deleted_at`-klausul, så "præcis ét default-mål pr. konto" holdes af konvention frem for af skemaet — og racen på indexet er nu logget (P3-59 fase 5), så vi kan for første gang se hvor ofte den fyrer | goal | S | open | [P3-59s review](../plans/2026-07-31-p359-request-path-logging.md#items-der-spawnes-ikke-løses-her) |
| P3-62 | **`create_goal` gjorde to round-trips til account-service, hvor den anden var død.** `exists()` er fjernet fra port, adapter og service; owner-opslaget ejer nu både existence, ownership og 404/503-opdelingen, og tests fastholder præcis ét kald før write | goal | S | **done 2026-08-01** | [plan + Outcome](../plans/2026-08-01-p360-p362-upstream-error-honesty.md#outcome-fill-in-when-done) |
| P3-63 | **goal: audit-trailet er bevaret i DB'en men uopnåeligt gennem API'et.** `get_allocation_history` kalder `get_by_id` først (`service.py:164`), som filtrerer `deleted_at IS NULL` → 404 for et soft-deleted mål. P3-16 beholdt allokerings-rækkerne netop for at bevare historikken, så data findes — den kan bare ikke læses. Beslut om historik for slettede mål skal være tilgængelig, og for hvem | goal | S | open | [P3-59s review](../plans/2026-07-31-p359-request-path-logging.md#items-der-spawnes-ikke-løses-her) |
| P3-64 | **`shared/auth`s 401-warning ligger uden for `app.*` og misses af platformens egen søgning.** Loggeren er `auth.fastapi`. Målt i P3-59s efter-kørsel: `grep '\[app\.'` finder 14 linjer, `grep WARNING` finder 15 — den manglende er præcis den ene linje der dækker alle ~10 services. Samme fejlform som `execute_with_logging`s `analytics.usecase`. Valget er ikke oplagt (`auth.*` er pakkens naturlige modulnavn), så det er en beslutning: skal delte pakker logge under `app.*`, eller skal en log-gate matche på niveau frem for loggernavn? Hører sammen med P3-11 | observability, cross | S | open | [P3-59s Outcome](../plans/2026-07-31-p359-request-path-logging.md#outcome-2026-07-31) |
| P3-65 | Gør dev-notes retrieval afgrænset og Codex-ready: tracked `AGENTS.md`, canonical `.agents/skills`, kort STATUS/indeks, ens frontmatter og gates mod ny kontekstdrift. | docs, DX, agents | M | **done 2026-08-01** | [plan + Outcome](../plans/2026-08-01-p365-codex-dev-notes-optimization.md#outcome) |
...
**Korrektion 2026-07-29, fra [planens](../plans/2026-07-29-p228-taxonomy-internal-only.md) undersøgelse:** to af påstandene ovenfor er upræcise. `DELETE` er allerede vagtet (`category_service.py:146` afviser en kategori med subkategorier; `:264` afviser `"Anden"`, merchant-mappings og regler), så det der kan slettes er tomme kategorier og blad-subkategorier. Den uvagtede fan-out er **`PUT`**: `update_category` har nul guards, og `analytics/projections.py:143` kalder `propagate_category_rename`, som omskriver `category_name` på alle brugeres transaktions-docs i ES. Beslutningen er truffet — internal-only, ikke en rollemodel — fordi perimeterens deny-backstop allerede lukker `/api/v1/internal/*`. **Lukket 2026-07-29**: de seks skrive-ruter ligger under `/api/v1/internal/…` bag `require_internal_api_key`; fravalgene er filed som F2-14 (admin-rolle) og F2-15 (per-bruger-kategorier)
...
**P3-48** (frontend-vagt). P3-49 er lukket i P2-42b.
