...
| P2-43 | **En klient kan gøre en goal-række permanent 500 med ét PUT.** `goal-service/app/application/dto.py:15` har `status: Optional[str]` uvalideret, mens `GoalResponse.status` er `GoalStatus` (`dto.py:38`) og `_to_dto` (`service.py:204`) bygger modellen direkte. `PUT /goals/{id}` med `{"status": "bogus"}` passerer 422, **committes**, og kaster derefter `ValidationError` → 500 ved *hvert* efterfølgende `GET` af målet. Klient-trigget, persistent, og kun oprydning i DB'en får rækken tilbage. Tieren følger klassen, ikke datoen — samme admissionsregel P1-13 brugte. Fix: valider `status` på indgangen (enum i DTO'en), og beslut hvad der sker med de rækker der allerede står forkert | goal | S | open | fundet under [P3-59s review](../plans/2026-07-31-p359-request-path-logging.md#items-der-spawnes-ikke-løses-her) |
