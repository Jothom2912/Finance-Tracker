...
| P2-04 | Gateway async rewrite: shared AsyncClient per upstream, `asyncio.gather` for independent calls, per-request memoization of tx/taxonomy fetches, retry+breaker helper | gateway | M | partially done, rest deliberately rolled back (2026-07-12): memoization shipped; the async conversion was reverted to sync because master's ADR-0004 analytics stack is sync and the legacy read path is slated for post-cutover deletion — redo async (if still needed) after that cleanup | H13, H1/H2-gw, M11 |
