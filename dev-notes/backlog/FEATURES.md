...
| F1-07 | **Scheduled month-close (day-7)** — ADR-0003's original v1 trigger: auto-close previous budget month per account on day 7 | Months close without user discipline; kills the "closed too early on incomplete numbers" risk | Close endpoint + button exist (F1-04); pairs with F1-05's scheduler infra decision (worker loop vs KEDA cron) | F1-05 scheduler pattern (or build first here) | S | open |
