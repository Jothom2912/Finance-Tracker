    ...
    try:
        ...
        for category_name in set(category_rules.values()):
            if category_name.lower() not in existing_names:
                ...
                categories_to_add.append({"name": category_name, "type": category_type})

        # Sørg for at "Anden" kategori findes (fallback kategori)
        if "anden" not in existing_names and "anden" not in {cat.lower() for cat in set(category_rules.values())}:
            categories_to_add.append({"name": "Anden", "type": "expense"})
