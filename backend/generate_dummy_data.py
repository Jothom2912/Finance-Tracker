...
def generate_dummy_data(clear_existing: bool = False):
    ...
    db = SessionLocal()
    try:
        # Sørg for at tabeller eksisterer
        create_db_tables()
        ...
        for user in users:
            for i, account_name in enumerate(account_names[:2] if user.username == "johan" else account_names[:1]):
                account = Account(
        ...
        # Generer transaktioner for de sidste 6 måneder
        for account in accounts:
            # Expense transaktioner
            ...
            for _ in range(random.randint(2, 8)):
                ...
                transaction = Transaction(
                    amount=amount,
                    description=f"{category.name} - {transaction_date.strftime('%B %Y')}",
                    date=transaction_date,
                    type="income",
                    Category_idCategory=category.idCategory,
                    Account_idAccount=account.idAccount
        ...
        for account in accounts[:len(accounts)//2]:  # Budget for halvdelen af konti
            # Opret 2-4 budgetter per konto
            for _ in range(random.randint(2, 4)):
                ...
                # Tilføj kategori til budget via association table
                db.execute(
        ...
        for account in accounts[:len(accounts)//2]:  # Goals for halvdelen af konti
        ...
        for account in accounts[:len(accounts)//2]:
            for _ in range(random.randint(1, 3)):
                ...
                planned = PlannedTransactions(
                    Transaction_idTransaction=None,  # Ikke knyttet til eksisterende transaktion
        ...
        print(f"\n🔑 LOGIN INFORMATION:")
        print(f"  Username: johan / Password: test123")
        print(f"  Username: marie / Password: test123")
