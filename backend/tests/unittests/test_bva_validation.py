...
def test_budget_amount_boundary_values():
    ...
    from backend.budget.application.dto import BudgetCreate

    # -0.01 - INVALID
    with pytest.raises(ValidationError):
        BudgetCreate(category_id=1, amount=-0.01, start_date=date.today(), end_date=date.today() + timedelta(days=1))

    # 0 - VALID (grænse)
    valid = BudgetCreate(category_id=1, amount=0.00, start_date=date.today(), end_date=date.today() + timedelta(days=1))
    assert abs(valid.amount - 0.00) < 0.001

    # 0.01 - VALID
    valid = BudgetCreate(category_id=1, amount=0.01, start_date=date.today(), end_date=date.today() + timedelta(days=1))
...
def test_budget_period_boundary_values():
    ...
    for amount in [0.0, 0.01, 100.0, 1000.0]:
        valid = BudgetCreate(amount=amount, budget_date=today)
        assert abs(valid.amount - amount) < 0.001

    # Invalid amount (negative)
    with pytest.raises(ValidationError):
        BudgetCreate(amount=-0.01, budget_date=today)
...
def test_budget_date_boundary_values():
    ...
    valid = BudgetCreate(amount=100.0, budget_date=today)
    assert valid.budget_date == today

    valid = BudgetCreate(amount=100.0, budget_date=yesterday)
    assert valid.budget_date == yesterday

    valid = BudgetCreate(amount=100.0, budget_date=tomorrow)
    assert valid.budget_date == tomorrow

    # Valid without date (optional)
    valid = BudgetCreate(amount=100.0)
...
def test_goal_target_amount_boundary_values():
    ...
    from backend.goal.application.dto import GoalCreate

    # -0.01 - INVALID
    with pytest.raises(ValidationError):
        GoalCreate(Account_idAccount=1, target_amount=-0.01, current_amount=0)

    # 0 - VALID (grænse)
    valid = GoalCreate(Account_idAccount=1, target_amount=0.00, current_amount=0)
    assert abs(valid.target_amount - 0.00) < 0.001

    # 0.01 - VALID
    valid = GoalCreate(Account_idAccount=1, target_amount=0.01, current_amount=0)
...
def test_goal_current_vs_target_boundary():
    ...
    with pytest.raises(ValidationError):
        GoalCreate(Account_idAccount=1, target_amount=100, current_amount=101)

    # current = target - VALID (grænse)
    valid = GoalCreate(Account_idAccount=1, target_amount=100, current_amount=100)
    assert valid.current_amount == 100

    # current < target - VALID
    valid = GoalCreate(Account_idAccount=1, target_amount=100, current_amount=99)
...
def test_goal_deadline_boundary_values():
    ...
    with pytest.raises(ValidationError):
        GoalCreate(Account_idAccount=1, target_amount=100, current_amount=0, target_date=yesterday)

    # Dato i dag - INVALID
    with pytest.raises(ValidationError):
        GoalCreate(Account_idAccount=1, target_amount=100, current_amount=0, target_date=today)

    # Dato i morgen - VALID (grænse)
    valid = GoalCreate(Account_idAccount=1, target_amount=100, current_amount=0, target_date=tomorrow)
