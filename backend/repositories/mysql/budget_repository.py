...
class MySQLBudgetRepository(IBudgetRepository):
    ...
    def create(self, budget_data: Dict) -> Dict:
        try:
            ...
            if category_id:
                self.db.execute(
                    budget_category_association.insert().values(
                        Budget_idBudget=budget.idBudget, Category_idCategory=category_id
                    )
                )
            ...
            budget = (
                self.db.query(BudgetModel)
                .options(joinedload(BudgetModel.categories))
                .filter(BudgetModel.idBudget == budget.idBudget)
                .first()
            )
    ...
    def update(self, budget_id: int, budget_data: Dict) -> Dict:
        try:
            ...
            if category_id is not None:
                ...
                self.db.execute(
                    budget_category_association.insert().values(
                        Budget_idBudget=budget_id, Category_idCategory=category_id
                    )
                )
    ...
    def delete(self, budget_id: int) -> bool:
        try:
            budget = self.db.query(BudgetModel).filter(BudgetModel.idBudget == budget_id).first()
            if not budget:
                return False
