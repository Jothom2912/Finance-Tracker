...
class MySQLTransactionRepository(ITransactionRepository):
    ...
    def get_by_id(self, transaction_id: int) -> Optional[Dict]:
        try:
            transaction = (
                self.db.query(TransactionModel).filter(TransactionModel.idTransaction == transaction_id).first()
            )
    ...
    def get_summary_by_category(self, start_date: Optional[date] = None, end_date: Optional[date] = None) -> Dict:
        try:
            ...
            for t in transactions:
                category = (
                    self.db.query(CategoryModel).filter(CategoryModel.idCategory == t.Category_idCategory).first()
                )
