...
class MySQLCategoryRepository(ICategoryRepository):
    ...
    def get_by_id(self, category_id: int) -> Optional[Dict]:
        try:
            category = self.db.query(CategoryModel).filter(CategoryModel.idCategory == category_id).first()
    ...
    def create(self, category_data: Dict) -> Dict:
        try:
            category = CategoryModel(name=category_data.get("name"), type=category_data.get("type", "expense"))
    ...
    def _serialize_category(category: CategoryModel) -> Dict:
        return {"idCategory": category.idCategory, "name": category.name, "type": category.type}
