...
class ElasticsearchGoalRepository(IGoalRepository):
    ...
    def get_all(self, account_id: Optional[int] = None) -> List[Dict]:
        ...
        try:
            response = self.es.search(index=self.index, query=query, sort=[{"idGoal": "asc"}])
            goals = []
    ...
    def create(self, goal_data: Dict) -> Dict:
        ...
        if "idGoal" not in goal_data or goal_data.get("idGoal") is None:
            try:
                ...
                response = self.es.search(index=self.index, size=0, aggs={"max_id": {"max": {"field": "idGoal"}}})
        ...
        try:
            self.es.index(index=self.index, id=doc["idGoal"], document=doc, refresh=True)
    ...
    def update(self, goal_id: int, goal_data: Dict) -> Dict:
        ...
        try:
            self.es.index(index=self.index, id=goal_id, document=updated, refresh=True)
            return updated
        except Exception as e:
